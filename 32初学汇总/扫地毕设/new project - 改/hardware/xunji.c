...
void xunji_gpio()
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13 | GPIO_Pin_14| GPIO_Pin_15; //定义红外引脚
...
void run()
{
		TIM_SetCompare3(TIM3,175);//left
   		TIM_SetCompare4(TIM3,170);//right
	...
	if(R==1&&M==0&&L==0)
{
	...
	while(R==0&&M==0&&L==0&&b==0)
	{
		stop();
	}
	while(R==0&&M==1&&L==0&&b==0)
	{
		run();
	}
}
